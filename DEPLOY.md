...
EC2에는 git clone도, `.env.production` 같은 파일도 필요 없다 — Docker만 설치돼있으면 된다.
...
3. 이 값들을 `ENV_DOCKER` Secret에 채움 (아래 참고)
...
| `ENV_DOCKER` | `.env.docker.example`을 채운 내용 전체 (SECRET_KEY, Supabase 접속정보 등) |
...
CORS 자체가 발생하지 않는다(그 구성이 아니면 `.env.docker`의 `CORS_EXTRA_ORIGINS`를 채울 것).

## DB 접근 (운영 중 조회/디버깅)

Supabase 콘솔에서 바로 SQL 편집기/테이블 뷰로 확인 가능. 또는 `.env.docker`에 채운
...
2. `ENV_DOCKER`의 `ALLOWED_HOSTS`를 IP에서 도메인으로 교체 후 재배포
